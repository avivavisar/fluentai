...
class AiUnavailableError extends Error {
  statusCode = 503;
  constructor() {
    super('AI is not configured. Set ANTHROPIC_API_KEY in backend/.env.');
  }
}
...
export async function chatTurn(params: {
  ...
  const profileBlock = [
    ...
    scenario && scenario !== 'FREE' ? `- scenario/roleplay: ${scenario}` : `- mode: free conversation`,
  ].join('\n');
  ...
  return firstJson<ChatTurnResult>(resp.content);
}
...
export async function gradePlacement(params: {
  answers: { id: string; level: string; prompt: string; chosen: string; correct: string; isCorrect: boolean }[];
  ...
  const mcSummary = params.answers
    .map(
      (a) =>
        `[${a.level}] "${a.prompt}" chose: "${a.chosen}" (${a.isCorrect ? 'correct' : `wrong, answer: "${a.correct}"`})`,
    )
    .join('\n');
  ...
  return firstJson<PlacementGrade>(resp.content);
}

// ---------- Quick translation (English -> Hebrew) ----------
// Uses Haiku: translation is a simple task and the user is cost-sensitive.
