...
- **M1.7 — New Flutter foundation** ✅ (build verified; user confirms live sign-up) — rebuilt `frontend/lib` fresh: design system (calm indigo, RTL, dark mode), **gotrue** pure-Dart auth + localStorage session (swapped from `supabase_flutter` — its transitive `objective_c` native-assets hook breaks on the spaced Windows path), shared API client (Bearer), 4-tab shell, auth-gated go_router. *Test passed:* `flutter analyze` clean + `flutter build web` succeeds. Live sign-up → `/v1/me` = user runs it. *Dep:* M0.3.
- **M1.8 — Onboarding + placement (premium)** ✅ (build verified; user confirms live) — `features/{onboarding,placement,gate,profile}`. Onboarding: 4 premium steps (Maya welcome → goal → interests chips → Hebrew-support) → `POST /v1/profile/onboarding`. Placement: questions + writing → `POST /v1/placement/submit` → CEFR result. `GateScreen` routes onboarding→placement→shell off `/v1/profile`. **Design:** Warm Violet & Peach palette + Varela Round font + rounded cards. *Test passed:* `flutter analyze` clean + `build web` ok. Live flow = user runs it. *Dep:* M1.3, M1.7.
- **M1.9 — Signature "Today" home** ✅ (build verified) — `features/home/`: Maya greeting by name (time-aware), streak pill, big violet "דבר עם המורה" CTA, "המשימה של היום" cards, live stat cards from `/v1/progress` (`progressProvider`), RefreshIndicator, logout. Placeholder actions show "בקרוב" (wired in P2/M4). Matches the approved mockup. *Test passed:* `flutter analyze` clean + `build web`. Shared live via cloudflared for user/brother feedback. *Dep:* M1.6, M1.7.
- **M1.10 — Premium bilingual chat** ✅ (build verified) — `features/chat/` (`chat_screen.dart` + `widgets/correction_card.dart`): message bubbles (learner LTR-in-RTL), typing indicator, empty-state, correction cards (original→suggestion + EN, on-demand "תרגום"→HE), new-vocab chips; wired to the שיחה tab + home CTA via `shellTabProvider`; uses `companionProvider` for the tutor's name. *Test passed:* `flutter analyze` clean + `build web`; live on the shared link. *Dep:* M1.4, M1.7. **→ Phase 1 COMPLETE (M1.1–M1.11).**
- **M1.11 — User gender + tutor selection** ✅ (build verified) — `Profile.gender` (migration); `src/companion/` (6 presets Maya/Noa/Emily/Ethan/Daniel/Jack, `GET /v1/companions/presets`, `POST/GET /v1/companion`); onboarding gained gender + "choose your tutor" steps (**all tutors shown to everyone**); chosen persona + learner gender injected into `AiService.chatTurn`; home greets with the chosen tutor's name; placement writing-feedback de-named. Per-tutor **voice** deferred to P2. *Test passed:* backend e2e — presets, onboarding gender=MALE, select Ethan → chat reply in Ethan's casual persona + go→went with HE explanation; `flutter analyze` + `build web` clean. *Dep:* M1.8.
